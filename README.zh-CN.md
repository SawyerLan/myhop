...
当 `myhop` 退回到原生 `mysql` 客户端（没装 mycli 的情况）时，会始终带上 `--comments` 参数。如果不加这个参数，`mysql` 客户端的默认行为（`--skip-comments`）会在把语句发给服务端之前，先在客户端把 SQL 注释剥掉——包括 TiDB 的 `/*T! ... */` 注释，这类注释里藏着 TiDB 专属的 DDL 属性（`SHARD_ROW_ID_BITS`、`AUTO_RANDOM`、placement policy、TTL 等），之所以用注释语法包起来，就是为了兼容普通 MySQL 工具链。在没加 `--comments` 的 `mysql` 会话里执行 `source some.sql`，这些属性会被静默丢弃——DDL 照样执行成功，只是属性没生效，而且不会有任何报错提示你发生了什么。`mycli` 没有这个问题（它不做这种客户端侧的注释剥离），所以这个坑只存在于 `mysql` 这条退回路径上。
